...
- Fails (non-zero) if the API submission returns a status code not in
  `fact_inventory_gather_api_expected_status_codes` (default: `[201]`).
- The API POST uses `failed_when: false` so that the subsequent audit
  logging and validation tasks always run. Validation then fails the host
  if the response status is not in the expected list.
