...
- **Rate Limiting**: Per-IP rate limiting (default: 2 requests per hour per IP)
...
(nginx, Apache, or Kubernetes Ingress) typically strips the `/fact_inventory` prefix
before forwarding requests to the application. This design supports three deployment modes:
...
**Data Retention**
Purges records older than `RETENTION_DAYS`. Runs after `RETENTION_CHECK_INTERVAL_HOURS` plus random jitter. First run deferred until after the initial interval and jitter; no cleanup work runs during application startup.

**Duplicate Pruning**
Removes duplicates per `client_address`, keeping the newest `HISTORY_MAX_ENTRIES`. Runs after `HISTORY_CHECK_INTERVAL_HOURS` plus random jitter. First run deferred until after the initial interval and jitter; no cleanup work runs during application startup.

## Security Model

- No client authentication or network restrictions
- Rate limiting by IP address (can be bypassed with IP rotation; default: 2 requests per hour per IP)
...
For detailed API usage, including examples with different `APP_PREFIX` settings,
see the client documentation in `docs/CLIENT.md`.
