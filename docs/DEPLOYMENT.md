...
For CPU-bound workloads, use the default sync workers with gunicorn managing multiple uvicorn instances:

```bash
# Default: Gunicorn spawns multiple uvicorn workers
WEB_CONCURRENCY=4 gunicorn \
  fact_inventory:app_factory \
  --factory \
  --worker-class uvicorn.workers.UvicornWorker \
  --bind 0.0.0.0:8000 \
...
```bash
gunicorn fact_inventory:app_factory --factory -c gunicorn.conf.py
```

This binds to `http://0.0.0.0:8000` with 8 workers. In production, the reverse proxy strips the `/fact_inventory` prefix, so the application runs at `/` internally (e.g., receives `/api/v1/facts`). For direct development access to Uvicorn, you can override the prefix (see below).

### Expected request paths when Uvicorn runs standalone

When accessing Uvicorn directly (no proxy in front), use these paths:

| Request Path                   | Handler                   | Enabled by default | Feature Flag             |
| ------------------------------ | ------------------------- | ------------------ | ------------------------ |
| `/fact_inventory/api/v1/facts` | `FactInventoryController` | Yes                | N/A                      |
| `/fact_inventory/health`       | `health_check()`          | No                 | `ENABLE_HEALTH_ENDPOINT` |
| `/fact_inventory/ready`        | `ready_check()`           | No                 | `ENABLE_READY_ENDPOINT`  |
| `/fact_inventory/metrics`      | Prometheus metrics        | No                 | `ENABLE_METRICS`         |
