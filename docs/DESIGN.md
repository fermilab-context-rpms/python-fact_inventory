...
The standalone application factory (`app_factory`) serves them at the root path `/`
by default (suitable for production behind a reverse proxy that strips the prefix).

When embedding in a larger Litestar application, pass a custom prefix to `create_router`:
...
| `sa_orm_sentinel` | INTEGER          | Parallel update guard (auto-maintained)           |
...
The `AsyncBackgroundJobPlugin` (`fact_inventory/server/background.py`) implements
