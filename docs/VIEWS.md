...
Views are read-only projections with no storage overhead and reflect
...
address per interface per host. Excludes loopback (127.0.0.0/8).
...
interface per host. Excludes loopback (::1).
