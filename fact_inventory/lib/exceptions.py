...
class FactValidationError(ValueError):
    ...
    local_facts, client_facts) are empty, which violates the rule that
    a submission must contain at least one category of data.
    """


class FactPayloadTooLargeError(ValueError):
    """Raised when a submitted JSON field exceeds the configured size limit."""
