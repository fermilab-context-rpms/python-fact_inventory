...
class AsyncBackgroundJobPlugin(InitPlugin):
    ...
    @property
    def job_callback(self) -> Callable[..., Coroutine[Any, Any, int]]:
        """Return the scheduled job callback."""
        return self._job_callback

    @property
    def interval_seconds(self) -> int:
        """Return the configured execution interval in seconds."""
        return self._interval

    @property
    def jitter_seconds(self) -> int:
        """Return the configured maximum random delay in seconds."""
        return self._jitter

    @property
    def name(self) -> str:
        """Return the human-readable job name used for logs and task names."""
        return self._name
    ...
    def on_app_init(self, app_config: AppConfig) -> AppConfig:
        ...
        Required by InitPluginProtocol. Appends lifespan to
    ...
    # ----------------------------------------------------------------------
    @asynccontextmanager
    async def lifespan(self, _app: Litestar) -> AsyncGenerator[None, None]:
        ...
        asyncio.Task wrapping run_forever, yields control to the running
        ...
        """
        task = asyncio.create_task(self.run_forever(), name=self.name)
        try:
            yield
        finally:
            task.cancel()
    ...
    async def run_forever(self) -> None:
        ...
        while True:
            ...
            if self.jitter_seconds:
                jitter = secrets.randbelow(int(self.jitter_seconds + 1))
            ...
            try:
                ...
                logger.exception("Background job failed, rescheduling")
