...
class FactInventoryController(Controller):
    ...
    request_max_body_size: int | None | EmptyType = (
        settings.max_request_body_mb * 1024 * 1024
    )

    @post(
        "",
        dto=FactInventoryWriteAPI,
        description="Submit system, package, local, and agent facts",
        responses={
    ...
    async def submit(
                ...
                examples=[
                    Example(
                        ...
                        value={
                            ...
                            "agent_facts": {"target_url": "/example/path"},
                        },
                    ),
                    Example(
        ...
        try:
            await fact_inventory_service.insert_record(
                data={
                    ...
                    "agent_facts": data.agent_facts,
                }
            )
        except HTTPException:
            raise
        except FactValidationError as err:
            self._handle_error(err, request, HTTP_400_BAD_REQUEST)
        except SQLAlchemyError as err:
            self._handle_error(err, request, HTTP_409_CONFLICT)
        except ValueError as err:
            self._handle_error(err, request, HTTP_413_REQUEST_ENTITY_TOO_LARGE)
        except TimeoutError as err:
            self._handle_error(err, request, HTTP_504_GATEWAY_TIMEOUT)
        except Exception as err:
            self._handle_error(err, request, HTTP_500_INTERNAL_SERVER_ERROR)
