...
@get(
    ...
    responses={
        ...
        HTTP_503_SERVICE_UNAVAILABLE: ResponseSpec(
            data_container=None,
            description=(
                "Service unavailable. Raised as an HTTPException, so the body"
                " uses Litestar's default error shape"
            ...
            examples=[
                Example(
                    ...
                    value={"status_code": 503, "detail": "Service unavailable"},
                )
            ],
        ),
    },
...
async def ready_check(
    ...
    be manually closed by the handler. Failures are logged server-side and a
    generic 503 response is returned without database details.
    ...
    try:
        ...
        return Response(content={"status": "ready"}, status_code=HTTP_200_OK)
