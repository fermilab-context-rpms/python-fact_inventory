...
def create_router(path: str = "/", settings: Settings | None = None) -> Router:
    ...
    settings : Settings | None
        Application configuration. Resolved via ``get_settings()`` when not
        supplied; determines which optional endpoints are registered.
    ...
    route_handlers: list[Any] = [root_handler, create_api_router(settings=resolved)]

    if resolved.enable_health_endpoint:
        route_handlers.append(health_check)

    if resolved.enable_ready_endpoint:
