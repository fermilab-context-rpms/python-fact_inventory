...
class FactInventoryService(
    ...
    async def purge_fact_history_more_than(self, max_entries: int) -> int:
        ...
        if deleted_count > 0:
            logging.getLogger(__name__).info(
                "%i Excess fact history removed per client", deleted_count
            )
        return deleted_count
    ...
    async def upsert_client_record(self, data: dict[str, Any]) -> FactInventory:
        ...
        -------
        FactInventory
            The created or updated record with database-generated fields
            populated.
        ...
        """
        self._validate_json_field_constraints(data)
        record = await self.repository.upsert_client_record(data)
        logging.getLogger(__name__).info(
