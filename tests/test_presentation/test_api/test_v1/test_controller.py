...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("agent_facts", True, id="boolean"),
    ],
)
...
async def test_payload_size_limits_field_over_json_limit_returns_413(
    ...
    response = await post_facts(
        test_client,
        payload=create_payload_with_oversized_field(settings.max_json_field_mb),
    )
...
async def test_payload_size_limits_field_at_limit_accepted(
    ...
    response = await post_facts(
        test_client,
        payload=create_payload_with_field_at_limit(settings.max_json_field_mb),
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            lambda: create_payload_over_body_limit(settings.max_request_body_mb),
            HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            id="body-over-limit",
        ),
        pytest.param(
