...
def assert_import_requires_deployment(stderr: str) -> None:
    """Assert the settings import failure explains the missing DEPLOYMENT."""
...
def test_settings_defaults_has_app_prefix() -> None:
    ...
    assert settings.app_prefix == "/fact_inventory"


def test_settings_requires_deployment() -> None:
    """Settings module raises KeyError when DEPLOYMENT env var is missing."""
    result = subprocess.run(
        [sys.executable, "-c", "from fact_inventory.lib.settings import settings"],
        env={},
        capture_output=True,
        text=True,
        check=False,
