...
ServiceWriteOperation = Callable[
    [FactInventoryService, dict[str, Any]], Awaitable[FactInventory]
]
...
SINGLE_FACT_CASES = [
    ...
    pytest.param(("agent_facts", {"target_url": "/example/path"}), id="agent_facts"),
]


async def upsert_record(
    service: FactInventoryService, payload: dict[str, Any]
) -> FactInventory:
    return await service.upsert_client_record(payload)


async def insert_record(
    service: FactInventoryService, payload: dict[str, Any]
) -> FactInventory:
    return await service.insert_record(payload)


@pytest.fixture(
    params=[
        pytest.param(upsert_record, id="upsert"),
        pytest.param(insert_record, id="insert"),
    ]
)
def write_operation(request: pytest.FixtureRequest) -> ServiceWriteOperation:
    """Return the service write path under test."""
    return request.param


@parametrize_ipv4_ipv6()
async def test_upsert_record_creates_new_record(
    test_db_session: AsyncSession,
    service: FactInventoryService,
    test_ips: dict[str, str],
) -> None:
    """Upsert creates a record when the client is new."""
    record = await service.upsert_client_record(
        create_record(
            test_ips["first"],
    ...
    assert record.client_address == test_ips["first"]
    assert record.system_facts == {"os": "RHEL"}
    assert record.id is not None


@parametrize_ipv4_ipv6()
async def test_upsert_record_updates_existing_record(
    test_db_session: AsyncSession,
    service: FactInventoryService,
    test_ips: dict[str, str],
) -> None:
    """Upsert replaces facts in place for an existing client."""
    client_address = test_ips["second"]
    original_record = await service.upsert_client_record(
        create_record(client_address, system_facts={"v": "1"})
    )

    updated_record = await service.upsert_client_record(
        create_record(client_address, system_facts={"v": "2"})
    )

    assert updated_record.id == original_record.id
    assert updated_record.system_facts == {"v": "2"}


@parametrize_ipv4_ipv6()
async def test_upsert_record_preserves_id(
    test_db_session: AsyncSession,
    service: FactInventoryService,
    test_ips: dict[str, str],
) -> None:
    """Repeated upserts preserve the same database row."""
    ...
    first = await service.upsert_client_record(payload)
    second = await service.upsert_client_record(payload)

    assert second.id == first.id


@parametrize_ipv4_ipv6()
async def test_upsert_record_refreshes_updated_at(
    test_db_session: AsyncSession,
    service: FactInventoryService,
    test_ips: dict[str, str],
) -> None:
    """Repeated upserts refresh updated_at."""
    client_address = test_ips["upsert"]
    original = await service.upsert_client_record(
        create_record(client_address, system_facts={"v": "1"})
    )
    original_updated_at = original.updated_at

    await asyncio.sleep(0.1)
    updated = await service.upsert_client_record(
        create_record(client_address, system_facts={"v": "2"})
    )
...
@parametrize_ipv4_ipv6()
async def test_upsert_record_maintains_one_record_per_ip(
    test_db_session: AsyncSession,
    service: FactInventoryService,
    test_ips: dict[str, str],
) -> None:
    """Repeated upserts do not create duplicate rows."""
    client_address = test_ips["second"]
    payload = create_record(
        client_address,
        system_facts={"os": "RHEL"},
    ...
    await service.upsert_client_record(payload)
    await service.upsert_client_record(payload)
    await service.upsert_client_record(payload)
...
async def test_service_write_rejects_all_empty_facts(
    service: FactInventoryService,
    test_ips: dict[str, str],
    write_operation: ServiceWriteOperation,
) -> None:
    """Both write paths reject payloads with no facts."""
...
async def test_service_write_rejects_oversized_facts(
    ...
    """Both write paths enforce per-field JSON size limits."""
    payload = create_record(test_ips["first"])
    payload[fact_field] = {"data": create_oversized_field(settings.max_json_field_mb)}

    with pytest.raises(ValueError):
        await write_operation(service, payload)
...
async def test_service_write_accepts_field_at_limit(
    ...
    """Both write paths accept fields exactly at the configured limit."""
    at_limit = create_at_limit_field(settings.max_json_field_mb)
    client_address = test_ips["second"]

    record = await write_operation(
        service,
        create_record(
            client_address,
...
async def test_service_write_accepts_single_fact_type(
    ...
    """Both write paths accept payloads with exactly one populated category."""
    ...
    record = await write_operation(service, payload)
...
async def test_purge_facts_over_limit_keeps_newest_per_ip(
    ...
    """History purge keeps only the newest rows for a client."""
    client_address = test_ips["first"]
    models = []
