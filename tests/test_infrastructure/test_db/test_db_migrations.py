...
def test_get_head_revision_honors_migrations_dir_override(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The migrations directory can be overridden via environment variable."""
    default_head = get_head_revision()

    monkeypatch.setenv(MIGRATIONS_DIR_ENV_VAR, "migrations")

    assert get_head_revision() == default_head
...
async def test_get_database_revision_requires_uri_without_engine() -> None:
    """A URI is required when no existing engine is supplied."""
    with pytest.raises(ValueError, match="database_uri or engine is required"):
        await get_database_revision()
...
async def test_get_database_revision_uses_supplied_engine(tmp_path: Path) -> None:
    """Revision checks can reuse an application-owned engine."""
    db_uri = sqlite_db_uri(tmp_path)
    await setup_migration_table(db_uri, "abc123")
    engine = create_async_engine(db_uri)

    try:
        assert await get_database_revision(engine=engine) == "abc123"
    finally:
        await engine.dispose()
